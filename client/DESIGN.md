...
- **Whether axe is measuring the screen or the animation.** It caught the walk out once, on the
  ...
  missed. `A11Y-6` having no enforcer was a second reason to give it one, and slice 8 did: an app
  that honours `prefers-reduced-motion` has no animation to wait for, so for a reader who has
  asked for less motion this class of artifact cannot arise at all. `settleAnimations` stays,
  because the walk itself runs without the preference.
