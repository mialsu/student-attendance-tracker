...
| INV-5 | A merge may only combine two Students of the same Class | merging a Student of Class A into a Student of Class B succeeds, moving attendance across a Class boundary | `test:test_merge_different_classes_error`, and at the service layer `test:tests/test_service_student.py::TestMergeStudents::test_merging_across_classes_is_refused` plus `test_a_refused_cross_class_merge_moves_nothing`, which asserts both histories are still intact after the refusal, and since spec 0005 slice 4 `test:tests/test_logging_events.py::test_a_cross_class_merge_is_refused_and_the_line_names_inv_5`, which asserts the refusal reaches the log naming this rule | `student_service.merge_students` — the same-Class comparison, which since spec 0005 slice 4 also labels the refusal `rule="INV-5"` so it reaches the log. Named by **function rather than file:line**, per INV-1's row and for the same evidence: this cell said `:515`, which pointed at the `if not duplicate_student:` guard two lines above the comparison it claimed to name. **`verify_class_ownership` on the duplicate's Class runs before this comparison**, and that order is load-bearing: while it ran after, a merge reaching into another teacher's Class was refused by the comparison and logged as `INV-5` for a violation that was `INV-1`'s. Reordered 2026-09-10 on the Owner's decision, found by spec 0005 slice 4's review. Reaching the comparison now means both Classes are the Teacher's own, so this row's rule is the one that refused. The other side is `test:tests/test_authorization.py::test_other_teacher_cannot_supply_her_own_student_as_a_merge_duplicate` and `test:tests/test_logging_events.py::test_another_teachers_student_as_the_duplicate_is_refused_as_inv_1` |
...
| INV-9 | **A log line this application emits identifies people by opaque id — no Student's name appears in one** | a captured line carries a Student's name, from a query string, a request body, or an exception's message. A name reaching a line is not recoverable: the sink rotates by SIZE only (`max-size: 10m`, `max-file: 3`), so at this app's volume a name written today effectively never ages out | `test:tests/test_logging_inv9.py` — the AC-5 sweeps: **seven** name-bearing routes × the denied and permitted paths, plus the `ERROR` line. Both directions are needed and the reason is in spec 0005 delta 14 — a refused request never runs the code that handles the name, and a permitted one emits no line, so either sweep alone proves almost nothing. Proven by plants, since the tests were green when written: six leaks, one per route plus the error line, each reddening the case that covers it (the four service-level ones redden the *permitted* case, which is the only one that executes them). **And** `gate:scripts/drift-extra.sh` check 5, which fails the *diff* so a leak is unlandable rather than merely noticed: `scripts/log_lint.py` parses the file with `ast` and checks logger-call keywords against an **allowlist**. Two earlier designs were defeated by review and both are recorded in spec 0005 delta 15 — an awk pass over diff text went clean when the call's opening line was not itself added, and a denylist of name-ish words went clean on `who=`, `search=` and `students=names` | `app/core/logging.py` — the formatter adds request context and nothing else, and every call site passes ids and counts. Two decisions do the real work and both are easy to undo by accident, which is why the gate exists: `app/api/handlers.py` never logs `exc.detail` (the one field carrying free text a name reaches), and the denial label on `BadRequestException` is **opt-in**, so the two refusals that interpolate a name into their message stay silent by construction. **Scope, and it is deliberate:** lines this app emits, not the whole system. `deployment/scripts/backup-db.sh` keeps seven `pg_dump`s including names, and SQLAlchemy renders bound parameters into the exception it raises, so five `LOWER(name) LIKE` sites can print a partial name in a *traceback* beside these lines. A wording covering those would have been false the day it was written — the pseudo-artifact this project names. The traceback gap is in `REVIEW-DEBT.md` (2026-09-10) with the `hide_parameters` trade it turns on; the backup gap is argued in `docs/adr/0007-personal-data-in-logs.md`, which is also where the wording above comes from |
